...
trait OldTastyInspector:
      ...
      def checkFile(fileName: String, ext: String): Unit =
         ...
         if file.extension != ext then
            throw new IllegalArgumentException(
              s"File extension is not `.$ext`: $file"
            )
         else if !file.exists then
            throw new IllegalArgumentException(
              s"File not found: ${file.toAbsolute}"
            )
   ...
   protected[inspector] def inspectFilesInContext(
      ...
      inspectorDriver().process(
        inspectorArgs(classpath, classes),
        summon[Context]
      )
   ...
   private def inspectorDriver() =
      ...
      class TastyInspectorPhase extends Phase:
         ...
         override def run(implicit ctx: Context): Unit =
            val qctx = QuotesImpl()
            self.processCompilationUnit(using qctx)(
              ctx.compilationUnit.tpdTree.asInstanceOf[qctx.reflect.Tree]
            )
      ...
      class TastyFromClass extends TASTYCompiler:
         ...
         override protected def backendPhases: List[List[Phase]] =
            List(
              new TastyInspectorPhase
            ) :: // Perform a callback for each compilation unit
               List(
                 new TastyInspectorFinishPhase
               ) :: // Perform a final callback
               Nil
         ...
         override def newRun(implicit ctx: Context): Run =
            ...
            new TASTYRun(
              this,
              ctx.fresh.addMode(Mode.ReadPositions).addMode(Mode.ReadComments)
            )

      new InspectorDriver

   private def inspectorArgs(
       classpath: List[String],
       classes: List[String]
   ): Array[String] =
   ...
   private def inspectFiles(
       classpath: List[String],
       classes: List[String]
   ): Boolean =
      if (classes.isEmpty)
         throw new IllegalArgumentException(
           "Parameter classes should no be empty"
         )

      val reporter =
         inspectorDriver().process(inspectorArgs(classpath, classes))
      reporter.hasErrors
