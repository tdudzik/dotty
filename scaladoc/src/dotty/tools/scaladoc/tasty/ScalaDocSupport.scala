...
trait ScaladocSupport { self: TastyParser =>
   ...
   def parseCommentString(
       comment: String,
       sym: Symbol,
       pos: Option[Position]
   ): Comment =
      val preparsed =
         comments.Preparser.preparse(comments.Cleaner.clean(comment))
         ...
         preparsed.syntax.headOption match {
            case Some(commentSetting) =>
               CommentSyntax.parse(commentSetting).getOrElse {
                  val msg =
                     s"not a valid comment syntax: $commentSetting, defaulting to Markdown syntax."
   ...
   def parseComment(docstring: String, tree: Tree): Comment =
      val commentString: String =
         if tree.symbol.isClassDef || tree.symbol.owner.isClassDef then
            ...
            comments.CommentExpander
               .cookComment(sym)(using ctx)
               .get
               .expanded
               .get
         else docstring
