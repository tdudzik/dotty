...
trait ClassLikeSupport:
  ...
  private def bareClasslikeKind(symbol: Symbol): Kind =
    ...
    else if symbol.flags.is(Flags.Enum) && symbol.flags.is(Flags.Case) then
      Kind.EnumCase(Kind.Object)
  ...
  private def kindForClasslike(classDef: ClassDef): Kind =
    ...
    def args = if constructorWithoutParamLists(classDef) then Nil
    else
      val constr =
      ...
      constr.fold(Nil)(
        _.termParamss.map(pList =>
          ParametersList(
            pList.params.map(mkParameter(_, parameterModifier)),
            if isUsingModifier(pList.params) then "using " else ""
          )
        )
      )
    ...
    else if classDef.symbol.flags.is(Flags.Enum) && classDef.symbol.flags.is(
      Flags.Case
    ) then Kind.EnumCase(Kind.Class(typeArgs, args))
  ...
  def mkClass(classDef: ClassDef)(
    ...
    def unpackTreeToClassDef(tree: Tree): ClassDef = tree match
      ...
      case TypeDef(_, tbt: TypeBoundsTree) =>
        unpackTreeToClassDef(tbt.tpe.typeSymbol.tree)
      case TypeDef(_, tt: TypeTree) =>
        unpackTreeToClassDef(tt.tpe.typeSymbol.tree)
      case c: Apply =>
        c.symbol.owner.tree.symbol.tree match
          case tree: ClassDef => tree
      case tt: TypeTree => unpackTreeToClassDef(tt.tpe.typeSymbol.tree)

    def getSupertypesGraph(
        classDef: Tree,
        link: LinkToType
      ...
      parents.flatMap { case tree =>
        val symbol =
          if tree.symbol.isClassConstructor then tree.symbol.owner
          else tree.symbol
        val superLink =
          LinkToType(tree.asSignature, symbol.dri, bareClasslikeKind(symbol))
    ...
    val graph = HierarchyGraph.withEdges(
      getSupertypesGraph(
        ...
        LinkToType(
          selfSiangture,
          classDef.symbol.dri,
          bareClasslikeKind(classDef.symbol)
        )
      )
    )

    val baseMember =
      mkMember(classDef.symbol, kindForClasslike(classDef), selfSiangture)(
        modifiers = modifiers,
        graph = graph,
        deprecated = classDef.symbol.isDeprecated()
      )

    if signatureOnly then baseMember
    else
      baseMember.copy(
  ...
  def extractImplicitConversion(tpe: TypeRepr): Option[ImplicitConversion] =
    if tpe.derivesFrom(conversionSymbol) then
      tpe.baseType(conversionSymbol) match
  ...
  private def parseMember(c: ClassDef)(s: Tree): Option[Member] = processTreeOpt(
    s
  )(s match
    ...
    case dd: DefDef
        if !dd.symbol.isHiddenByVisibility && dd.symbol.isGiven && !dd.symbol.isArtifact =>
      Some(dd.symbol.owner.memberType(dd.name))
        ...
        .map { _ =>
          parseMethod(
            c,
            dd.symbol,
            specificKind = Kind.Given(_, getGivenInstance(dd), None)
          )
        }

    case dd: DefDef
        if !dd.symbol.isHiddenByVisibility && dd.symbol.isExported && !dd.symbol.isArtifact =>
      ...
      val instanceName = exportedTarget
        .collect {
          case Select(qualifier: Select, _) => qualifier.name
          case Select(qualifier: Ident, _) =>
            qualifier.tpe.typeSymbol.normalizedName
        }
        .getOrElse("instance")
      val dri = dd.rhs
        .collect {
      ...
      Some(
        parseMethod(c, dd.symbol, specificKind = Kind.Exported(_))
          .withOrigin(Origin.ExportedFrom(s"$instanceName.$functionName", dri))
      )

    case dd: DefDef
        if !dd.symbol.isHiddenByVisibility && !dd.symbol.isGiven && !dd.symbol.isSyntheticFunc && !dd.symbol.isExtensionMethod && !dd.symbol.isArtifact =>
    ...
    case vd: ValDef
        if !isSyntheticField(vd.symbol)
          && (!vd.symbol.flags.is(Flags.Case) || !vd.symbol.flags
            .is(Flags.Enum))
          && vd.symbol.isGiven =>
      val classDef = Some(vd.tpt.tpe)
        .flatMap(_.classSymbol.map(_.tree.asInstanceOf[ClassDef]))
      Some(
        classDef
          .filter(_.symbol.flags.is(Flags.Module))
          .fold[Member](parseValDef(c, vd))(parseGivenClasslike(_))
      )
  ...
  private def parseGivenClasslike(c: ClassDef): Member = {
    ...
    parsedClasslike.withKind(
      Kind.Given(
        cls,
        givenParents.map(_.signature),
        parentTpe.flatMap(extractImplicitConversion)
      )
    )
  }

  private def parseInheritedMember(c: ClassDef)(s: Tree): Option[Member] =
    def inheritance = Some(
      InheritedFrom(s.symbol.owner.normalizedName, s.symbol.dri)
    )
    processTreeOpt(s)(s match
      case c: ClassDef
          if c.symbol.shouldDocumentClasslike && !c.symbol.isGiven =>
        Some(parseClasslike(c, signatureOnly = true))
      case c: ClassDef
          if c.symbol.owner
            .memberMethod(c.name)
            .exists(_.flags.is(Flags.Given)) =>
  ...
  extension (c: ClassDef)
    ...
    def getNonTrivialInheritedMemberTrees =
      c.symbol.getmembers
        ...
        .filter(s =>
          s.maybeOwner != defn.ObjectClass && s.maybeOwner != defn.AnyClass
        )
        .map(_.tree)
    ...
    def extractMembers: Seq[Member] = {
      val inherited = c.getNonTrivialInheritedMemberTrees.collect {
        case dd: DefDef
            if !dd.symbol.isClassConstructor && !(dd.symbol.isSuperBridgeMethod || dd.symbol.isDefaultHelperMethod) =>
          dd
        case other => other
      }
    ...
    /** Extracts members while taking Dotty logic for patching the stdlib into
      * account.
      */
    def extractPatchedMembers: Seq[Member] = {
      ...
      def extractPatchMembers(sym: Symbol) = {
        ...
        val ownMemberDRIs =
          ownMembers.iterator.map(_.name).toSet + "experimental$"
        sym.tree
          .asInstanceOf[ClassDef]
          .membersToDocument
          .filterNot(m => ownMemberDRIs.contains(m.symbol.name))
      ...
      c.symbol.fullName match {
        case "scala.Predef$" =>
          ownMembers ++
            extractPatchMembers(
              qctx.reflect.Symbol.requiredClass(
        ...
        case "scala.language$" =>
          ownMembers ++
            extractPatchMembers(
              qctx.reflect.Symbol
                .requiredModule("scala.runtime.stdLibPatches.language")
                .moduleClass
            )
        case "scala.language$.experimental$" =>
          ownMembers ++
            extractPatchMembers(
              qctx.reflect.Symbol
                .requiredModule(
                  "scala.runtime.stdLibPatches.language.experimental"
                )
                .moduleClass
            )
        case _ => ownMembers
    ...
    def getParentsAsLinkToTypes: List[LinkToType] =
      c.getParentsAsTreeSymbolTuples.map { (tree, symbol) =>
        LinkToType(tree.asSignature, symbol.dri, bareClasslikeKind(symbol))
      }

    def getParentsAsTreeSymbolTuples: List[(Tree, Symbol)] =
      for
        parentTree <- c.parents if isValidPos(
          parentTree.pos
        ) // We assume here that order is correct
        parentSymbol =
          if parentTree.symbol.isClassConstructor then parentTree.symbol.owner
          else parentTree.symbol
    ...
    def getConstructors: List[Symbol] = membersToDocument.collect {
      case d: DefDef
          if d.symbol.isClassConstructor && c.constructor.symbol != d.symbol =>
        d.symbol
    }.toList
    ...
    def getParameterModifier(parameter: Symbol): String =
      ...
      if fieldSymbol.flags.is(Flags.Mutable) then "var "
      else if fieldSymbol.flags.is(Flags.ParamAccessor) && !c.symbol.flags.is(
        Flags.Case
      ) && !fieldSymbol.flags.is(Flags.Private) then "val "
      else ""
  ...
  def parseClasslike(
      classDef: ClassDef,
      signatureOnly: Boolean = false
  ): Member = classDef match
    case c: ClassDef if classDef.symbol.flags.is(Flags.Module) =>
      parseObject(c, signatureOnly)
    case c: ClassDef
        if classDef.symbol.flags
          .is(Flags.Enum) && !classDef.symbol.flags.is(Flags.Case) =>
      parseEnum(c, signatureOnly)
  ...
  def parseObject(classDef: ClassDef, signatureOnly: Boolean = false): Member =
    mkClass(classDef)(
      ...
      modifiers =
        classDef.symbol.getExtraModifiers().filter(_ != Modifier.Final),
      signatureOnly = signatureOnly
    )

  def parseEnum(classDef: ClassDef, signatureOnly: Boolean = false): Member =
    val extraModifiers = classDef.symbol
      .getExtraModifiers()
      .filter(_ != Modifier.Sealed)
      .filter(_ != Modifier.Abstract)
    val companion =
      classDef.symbol.getCompanionSymbol.map(_.tree.asInstanceOf[ClassDef]).get

    val enumVals = companion.membersToDocument
      .collect {
        case vd: ValDef
      ...
      .toList
      .map(parseValDef(classDef, _))

    val enumTypes = companion.membersToDocument
      .collect {
        case td: TypeDef
    ...
    val enumNested = companion.membersToDocument.collect {
      case c: ClassDef
          if c.symbol.flags.is(Flags.Case) && c.symbol.flags.is(Flags.Enum) =>
        processTree(c)(parseClasslike(c))
    }.flatten

    val enumClass = mkClass(classDef)(
    ...
    val cases = (
      enumNested ++
        enumTypes ++
        enumVals.map(m =>
          m.copy(dri = m.dri.copy(location = enumClass.dri.location))
        )
    )
  ...
  def parseMethod(
    ...
    val genericTypes =
      if (methodSymbol.isClassConstructor) Nil else method.leadingTypeParams
        ...
        ParametersList(
          pList.params.map(
            mkParameter(
              _,
              paramPrefix,
              memberInfo = memberInfo.paramLists(index)
            )
          ),
          if isUsingModifier(pList.params) then "using " else ""
        )
      ...
      if methodSymbol.isClassConstructor then Kind.Constructor(basicKind)
        ...
        extractImplicitConversion(method.returnTpt.tpe) match
          case Some(conversion)
              if paramLists.size == 0 || (paramLists.size == 1 && paramLists.head.params.size == 0) =>
    ...
    val origin =
      if !methodSymbol.isOverriden then Origin.RegularlyDefined
      else
    ...
    mkMember(methodSymbol, methodKind, memberInfo.res.asSignature)(
      origin = origin,
  ...
  def mkParameter(
    ...
    val inlinePrefix =
      if argument.symbol.flags.is(Flags.Inline) then "inline " else ""
    val nameIfNotSynthetic = Option.when(
      !argument.symbol.flags.is(Flags.Synthetic)
  ...
  def parseTypeDef(typeDef: TypeDef): Member =
    ...
    val defaultKind = Kind
      .Type(!isTreeAbstract(typeDef.rhs), typeDef.symbol.isOpaque, generics)
      .asInstanceOf[Kind.Type]
    val kind =
      if typeDef.symbol.flags.is(Flags.Enum) then Kind.EnumCase(defaultKind)
      else defaultKind
    mkMember(typeDef.symbol, kind, tpeTree.asSignature)(deprecated =
      typeDef.symbol.isDeprecated()
    )
  ...
  def parseValDef(c: ClassDef, valDef: ValDef): Member =
    ...
    val kind =
      if valDef.symbol.flags.is(Flags.Implicit) then
    ...
    mkMember(valDef.symbol, kind, memberInfo.res.asSignature)(deprecated =
      valDef.symbol.isDeprecated()
    )
  ...
  case class MemberInfo(
      ...
      paramLists: List[Map[String, TypeRepr]],
      res: TypeRepr
  )
  ...
  def unwrapMemberInfo(c: ClassDef, symbol: Symbol): MemberInfo =
    ...
    def handlePolyType(polyType: PolyType): MemberInfo =
      MemberInfo(
        polyType.paramNames.zip(polyType.paramBounds).toMap,
        List.empty,
        polyType.resType
      )

    def handleMethodType(
        memberInfo: MemberInfo,
        methodType: MethodType
    ): MemberInfo =
      MemberInfo(
        memberInfo.genericTypes,
        memberInfo.paramLists ++ List(
          methodType.paramNames.zip(methodType.paramTypes).toMap
        ),
        methodType.resType
      )

    def handleByNameType(
        memberInfo: MemberInfo,
      ...
      MemberInfo(
        ...
        byNameType.underlying
      )

    def recursivelyCalculateMemberInfo(memberInfo: MemberInfo): MemberInfo =
      memberInfo.res match
        case p: PolyType => recursivelyCalculateMemberInfo(handlePolyType(p))
        case m: MethodType =>
          recursivelyCalculateMemberInfo(handleMethodType(memberInfo, m))
        case b: ByNameType => handleByNameType(memberInfo, b)
        case _             => memberInfo

    recursivelyCalculateMemberInfo(
      MemberInfo(Map.empty, List.empty, baseTypeRepr)
    )
