...
case class ScaladocTastyInspector()(using ctx: DocContext)
   ...
   def processCompilationUnit(using quotes: Quotes)(
   ...
   override def postProcess(using q: Quotes): Unit =
      ...
      inline def hackForeachTree(thunk: q.reflect.Tree => Unit): Unit =
         given dctx: dotc.core.Contexts.Context =
      ...
      val symbolsToSkip: Set[q.reflect.Symbol] =
         ctx.args.identifiersToSkip.flatMap { ref =>
            ...
            Try(qrSymbol.requiredPackage(ref)).orElse(
              Try(qrSymbol.requiredClass(ref))
            ) match {
      ...
      def isSkipped(sym: q.reflect.Symbol): Boolean =
         def isSkippedById(sym: q.reflect.Symbol): Boolean =
            ...
            else symbolsToSkip.contains(sym) || isSkipped(sym.owner)
      ...
      def driFor(link: String): Option[DRI] =
         ...
         Try(QueryParser(link).readQuery()).toOption.flatMap(query =>
            MemberLookup.lookupOpt(query, None).map {
               case (sym, _, inheritingParent) =>
                  inheritingParent match
                     case Some(parent) =>
                        sym.driInContextOfInheritingParent(parent)
                     case None => sym.dri
            }
         )
      ...
      def processRootDocIfNeeded(tree: parser.qctx.reflect.Tree) =
         def readFile(pathStr: String)(using CompilerContext): Option[String] =
            try
               ...
               case e: RuntimeException =>
                  report.warning(
                    s"Unable to read root package doc from $pathStr: ${throwableToString(e)}"
                  )
                  None
         ...
         if !alreadyProcessed then
            ...
            ctx.args.rootDocPath.flatMap(readFile).map { content =>
               ...
               def root(s: Symbol): Symbol = if s.owner.isNoSymbol then s
               else root(s.owner)
               val topLevelPck = root(tree.symbol)
               rootDoc =
                  Some(parser.parseCommentString(content, topLevelPck, None))
            }
         ...
         val intrinsicClassDefs = Seq(
           ...
           defn.orType
            ...
            "scala" -> s.asInstanceOf[parser.qctx.reflect.Symbol].tree.match {
               case cd: parser.qctx.reflect.ClassDef =>
                  parser.parseClasslike(cd)
   ...
   def result(): (List[Member], Option[Comment]) =
      ...
      val classpath =
         ctx.args.classpath.split(java.io.File.pathSeparator).toList
      ...
      all
         .groupBy(_._1)
         .map { case (pckName, members) =>
            val (pcks, rest) =
               members.map(_._2).partition(_.kind == Kind.Package)
            val basePck =
               pcks.reduce((p1, p2) =>
   ...
   def mergeAnyRefAliasAndObject(parser: TastyParser) =
      val defn = ctx.compilerContext.definitions
      val oM = parser.parseClasslike(
        defn.ObjectClass
           .asInstanceOf[parser.qctx.reflect.Symbol]
           .tree
           .asInstanceOf[parser.qctx.reflect.ClassDef]
      )
      val aM = parser.parseTypeDef(
        defn.AnyRefAlias
           .asInstanceOf[parser.qctx.reflect.Symbol]
           .tree
           .asInstanceOf[parser.qctx.reflect.TypeDef]
      )
      "scala" -> aM.copy(
        kind = oM.kind,
        members = oM.members
...
case class TastyParser(
    ...
    val ctx: DocContext
) extends ScaladocSupport
    with BasicSupport
    with TypesSupport
    with ClassLikeSupport
    with SyntheticsSupport
    with PackageSupport
    with NameNormalizer:
   import qctx.reflect._

   def processTree[T](tree: Tree)(op: => T): Option[T] = try Option(op)
   catch
      case e: Exception =>
         report.warning(throwableToString(e), tree.pos)
         None
   def processTreeOpt[T](tree: Tree)(op: => Option[T]): Option[T] = try op
   catch
      case e: Exception =>
         report.warning(throwableToString(e), tree.pos)
         None
   ...
   def parseRootTree(root: Tree): Seq[(String, Member)] =
      ...
      object Traverser extends TreeTraverser:
         ...
         override def traverseTree(tree: Tree)(owner: Symbol): Unit =
            seen = tree :: seen
            if !isSkipped(tree.symbol) then
               tree match {
                  case pck: PackageClause =>
                     docs += parsePackage(pck)
                     super.traverseTree(tree)(owner)
                  case packageObject: ClassDef
                      if (packageObject.symbol.name.contains("package$")) =>
                     docs += parsePackageObject(packageObject)
                  case clazz: ClassDef
                      if clazz.symbol.shouldDocumentClasslike =>
      ...
      try Traverser.traverseTree(root)(Symbol.spliceOwner)
         ...
         case e: Throwable =>
            println(
              s"Problem parsing ${root.pos}, documentation may not be generated."
            )
            e.printStackTrace()
