...
trait SyntheticsSupport:
  ...
  extension (t: TypeRepr)
    ...
    def isCompiletimeAppliedType: Boolean = hackIsCompiletimeAppliedType(using
      qctx
    )(t)
    ...
    def hackIsCompiletimeAppliedType(using Quotes)(
        rtpe: qctx.reflect.TypeRepr
    ): Boolean =
      import dotty.tools.dotc
      given ctx: dotc.core.Contexts.Context =
        qctx.asInstanceOf[scala.quoted.runtime.impl.QuotesImpl].ctx
  ...
  extension (s: Symbol)
    def isSyntheticFunc: Boolean = s.flags.is(Flags.Synthetic) || s.flags.is(
      Flags.FieldAccessor
    ) || isDefaultHelperMethod

    def isSuperBridgeMethod: Boolean = s.name.contains("$super$")

    def isDefaultHelperMethod: Boolean =
  ...
  def isSyntheticField(c: Symbol) =
    c.flags.is(Flags.CaseAccessor) || (c.flags.is(Flags.Module) && !c.flags.is(
      Flags.Given
    ))
  ...
  def constructorWithoutParamLists(c: ClassDef): Boolean =
    !isValidPos(c.constructor.pos) || {
      ...
      val typesEnd =
        c.constructor.leadingTypeParams.lastOption.fold(end - 1)(_.pos.end)
   ...
   */
  def hackGetmembers(using
      Quotes
  )(rsym: qctx.reflect.Symbol): List[qctx.reflect.Symbol] = {
    import qctx.reflect._
    import dotty.tools.dotc
    given ctx: dotc.core.Contexts.Context =
      qctx.asInstanceOf[scala.quoted.runtime.impl.QuotesImpl].ctx
    val sym = rsym.asInstanceOf[dotc.core.Symbols.Symbol]
    sym.typeRef
      .appliedTo(sym.typeParams.map(_.typeRef))
      .allMembers
      .iterator
      .map(_.symbol)
      .collect {
        case sym
            if (!sym.is(dotc.core.Flags.ModuleVal) || sym.is(
              dotc.core.Flags.Given
            )) &&
              !sym.flags.isAllOf(
                dotc.core.Flags.Enum | dotc.core.Flags.Case | dotc.core.Flags.JavaStatic
              ) =>
  ...
  def hackGetSupertypes(using Quotes)(rdef: qctx.reflect.ClassDef) = {
    ...
    val ref =
      classdef.symbol.info.asInstanceOf[dotc.core.Types.ClassInfo].appliedRef
  ...
  def typeForClass(c: ClassDef): TypeRepr =
    ...
    cSym.typeRef
      .appliedTo(cSym.typeParams.map(_.typeRef))
  ...
  def memberInfo(c: ClassDef, symbol: Symbol): TypeRepr =
    ...
    given dotc.core.Contexts.Context =
      qctx.asInstanceOf[scala.quoted.runtime.impl.QuotesImpl].ctx
    typeForClass(c)
      .asInstanceOf[dotc.core.Types.Type]
