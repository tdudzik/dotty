...
abstract class MarkupConversion[T](val repr: Repr)(using DocContext) {
   ...
   protected def filterEmpty(
       xs: SortedMap[String, String]
   ): SortedMap[String, T]

   val qctx: repr.qctx.type =
      if repr == null then null else repr.qctx // TODO why we do need null?
      ...
      if SchemeUri.matches(queryStr) then DocLink.ToURL(queryStr)
         ...
         QueryParser(queryStr).tryReadQuery() match
            case Left(err) =>
               ...
               report.warning(
                 s"Unable to parse query `$queryStr`: ${err.getMessage}"
               )
   ...
   final def parse(preparsed: PreparsedComment): Comment =
      ...
      Comment(
        ...
        throws = filterEmpty(preparsed.throws).view
           .mapValues(markupToDokka)
           .to(SortedMap),
        valueParams = filterEmpty(preparsed.valueParams).view
           .mapValues(markupToDokka)
           .to(SortedMap),
        typeParams = filterEmpty(preparsed.typeParams).view
           .mapValues(markupToDokka)
           .to(SortedMap),
        ...
        deprecated = single("@deprecated", preparsed.deprecated, filter = false)
           .map(markupToDokka),
        note = filterEmpty(preparsed.note).map(markupToDokka),
        example = filterEmpty(preparsed.example).map(markupToDokka),
        constructor =
           single("@constructor", preparsed.constructor).map(markupToDokka),
        group = single("@group", preparsed.group).map(markupToString),
        groupDesc = filterEmpty(preparsed.groupDesc).view
           .mapValues(markupToDokka)
           .to(SortedMap),
        groupNames = filterEmpty(preparsed.groupNames).view
           .mapValues(markupToDokka)
           .to(SortedMap),
        groupPrio = preparsed.groupPrio,
        hideImplicitConversions =
           filterEmpty(preparsed.hideImplicitConversions).map(markupToDokka)
      )
}
...
class MarkdownCommentParser(repr: Repr)(using DocContext)
   ...
   def stringToMarkup(str: String) =
      MarkdownParser.parseToMarkdown(
        str,
        markdown.DocFlexmarkParser(resolveLink)
      )
...
class WikiCommentParser(repr: Repr)(using DocContext)
   ...
   private def flatten(b: wiki.Block): String = b match
      ...
      case wiki.DefinitionList(items) =>
         items.headOption.fold("")(e => flatten(e._1))
