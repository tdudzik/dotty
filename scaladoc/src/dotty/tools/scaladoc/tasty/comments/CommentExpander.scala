...
class CommentExpander {
   ...
   def expandedDocComment(sym: Symbol, site: Symbol, docStr: String = "")(using
      ...
      val parent =
         if ((sym.is(Flags.Module) || sym.isClass) && site.is(Flags.Package))
            sym
         else site
            ...
            def sectionString(
               ...
               paramMap.get(param) match {
                  case Some(section) =>
                     ...
                     cleanupSectionText(
                       parent.substring(
                         sectionTextBounds._1,
                         sectionTextBounds._2
                       )
                     )
                  case None =>
                     println(
   ...
   protected def expandVariables(initialStr: String, sym: Symbol, site: Symbol)(
       using Context
   ): String = {
      val expandLimit = 10
               ...
               variableName(str.substring(vstart + 1, idx)) match {
                  case "super" =>
                     superComment(sym) foreach { sc =>
                        ...
                        replaceWith(
                          sc.substring(3, startTag(sc, superSections))
                        )
   ...
   def lookupVariable(vble: String, site: Symbol)(using
      ...
      case _ =>
         ...
         searchList collectFirst {
            case x if lookupDefs(x) contains vble => lookupDefs(x)(vble)
         } match {
...
object CommentExpander {
   ...
   private def expandComment(sym: Symbol, owner: Symbol, comment: Comment)(using
      ...
      val newComment = {
         val expandedComment =
            tplExp.expandedDocComment(sym, owner, comment.raw)
         Comment(
           comment.span,
           comment.raw,
           Some(expandedComment),
           Nil,
           tplExp.defs(sym)
         )
      }
