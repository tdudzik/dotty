...
trait MemberLookup {
   ...
   def lookupOpt(using Quotes, DocContext)(
      ...
      try
         ...
         def nearestPackage(sym: Symbol): Symbol =
            if sym.flags.is(Flags.Package) then sym
            else nearestPackage(sym.owner)

         def nearestMembered(sym: Symbol): Symbol =
            if sym.isClassDef || sym.flags.is(Flags.Package) then sym
            else nearestMembered(sym.owner)
            ...
            ownerOpt match {
               case Some(owner) =>
                  ...
                  def relativeLookup(
                     ...
                     if !isMeaningful then None
                     else {
                        downwardLookup(querystrings, owner) match {
                           case None =>
                              relativeLookup(querystrings, owner.owner)
                           case some => some
                        }
                     }
                  }
                  ...
                  query match {
                     case Query.StrictMemberId(id) =>
                        downwardLookup(List(id), nearest).map(
                          memberLookupResult(_, id, _)
                        )
                     case Query.QualifiedId(Query.Qual.This, _, rest) =>
                        downwardLookup(rest.asList, nearestCls).map(
                          memberLookupResult(_, rest.join, _)
                        )
                     case Query.QualifiedId(Query.Qual.Package, _, rest) =>
                        downwardLookup(rest.asList, nearestPkg).map(
                          memberLookupResult(_, rest.join, _)
                        )
                     case query =>
                        val ql = query.asList
               ...
               case None =>
                  toplevelLookup(query.asList).map(
                    memberLookupResult(_, query.join, _)
                  )
            }
         }
         ...
         case e: Exception =>
            ...
            val msg =
               s"Unable to find a link for ${query} ${ownerOpt
                  .fold("")(o => "in " + o.name)}"
            report.warn(msg, e)
            None
   ...
   private def hackMembersOf(using Quotes)(rsym: quotes.reflect.Symbol) = {
      ...
      given dotc.core.Contexts.Context =
         quotes.asInstanceOf[scala.quoted.runtime.impl.QuotesImpl].ctx
      val sym = rsym.asInstanceOf[dotc.core.Symbols.Symbol]
      val members =
         sym.info.allMembers.iterator
            .map(_.symbol)
            .filter(s => hackIsNotAbsent(s.asInstanceOf[Symbol]))
   ...
   private def hackIsNotAbsent(using Quotes)(rsym: quotes.reflect.Symbol) =
      ...
      iorc match
         case _: dotc.core.SymDenotations.ModuleCompleter |
             dotc.core.SymDenotations.NoCompleter | dotc.core.Types.NoType |
             _: dotc.core.NoLoader =>
            false
         case _ => true
   ...
   private def localLookup(using Quotes)(
      ...
      if owner.isPackageDef then
         findMatch(hackMembersOf(owner).flatMap { s =>
            (if s.name.endsWith("package$") then hackMembersOf(s)
             else Iterator.empty) ++ Iterator(s)
         })
      else
         owner.tree match {
            case tree: TypeDef =>
         ...
         case q :: Nil =>
            ...
            val res = sel.kind match {
               case MemberLookup.SelectorKind.NoForce =>
                  ...
                  lookedUp
                     .find(s => s.isType && !s.flags.is(Flags.Module))
                     .orElse(
...
object MemberLookup extends MemberLookup {
   ...
   object Selector {
      def fromString(str: String) = {
         ...
         else Selector(cleanStr, SelectorKind.NoForce)
      }
   }
}
