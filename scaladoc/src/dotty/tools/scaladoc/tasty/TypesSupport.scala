...
trait TypesSupport:
         ...
         def asSignature: DocSignature =
            tpeTree match
               case TypeBoundsTree(low, high) =>
                  typeBoundsTreeOfHigherKindedType(low.tpe, high.tpe)
   ...
   private def isRepeatedAnnotation(term: Term) =
      term.tpe match
         case t: TypeRef =>
            t.name == "Repeated" && t.qualifier.match
   ...
   private def inner(tp: TypeRepr): DocSignature =
      ...
      tp match
         ...
         case AndType(left, right) =>
            inner(left) ++ texts(" & ") ++ inner(right)
         case ByNameType(tpe) => text("=> ") :: inner(tpe)
         case ConstantType(constant) =>
            texts(constant.show)
         case ThisType(tpe) => inner(tpe)
         case AnnotatedType(AppliedType(_, Seq(tpe)), annotation)
             if isRepeatedAnnotation(annotation) =>
            inner(tpe) :+ text("*")
         case AppliedType(repeatedClass, Seq(tpe))
             if isRepeated(repeatedClass) =>
            inner(tpe) :+ text("*")
         ...
         case tl @ TypeLambda(params, paramBounds, resType) =>
            texts("[") ++ commas(params.zip(paramBounds).map { (name, typ) =>
               val normalizedName =
                  if name.matches("_\\$\\d*") then "_" else name
         ...
         case r: Refinement => { //(parent, name, info)
            def getRefinementInformation(t: TypeRepr): List[TypeRepr] =
               t match {
            ...
            def getParamList(m: MethodType): DocSignature =
               texts("(")
                  ++ m.paramNames
                     .zip(m.paramTypes)
                     .map { case (name, tp) => texts(s"$name: ") ++ inner(tp) }
                     .reduceLeftOption(
                       (acc: DocSignature, elem: DocSignature) =>
                          acc ++ texts(", ") ++ elem
                     )
                     .getOrElse(List())
                  ++ texts(")")

            def parseRefinedElem(
                name: String,
                info: TypeRepr,
                polyTyped: DocSignature = Nil
            ): DocSignature = (info match {
               case m: MethodType => {
                  val paramList = getParamList(m)
                  texts(s"def $name") ++ polyTyped ++ paramList ++ texts(
                    ": "
                  ) ++ inner(m.resType)
               }
               case t: PolyType => {
               ...
               case other =>
                  noSupported(s"Not supported type in refinement $info")
            }) ++ texts("; ")
            ...
            val refinedElems = refinementInfo.tail.collect {
               case r: Refinement =>
                  r
            }.toList
            val prefix =
               if refinedType.typeSymbol != defn.ObjectClass then
                  inner(refinedType) ++ texts(" ")
               else Nil
            if (
              refinedType.typeSymbol.fullName == "scala.PolyFunction" && refinedElems.size == 1
            ) {
               parsePolyFunction(refinedElems.head.info)
            } else {
               prefix ++ texts("{ ") ++ refinedElems.flatMap(e =>
                  parseRefinedElem(e.name, e.info)
               ) ++ texts(" }")
            }
         }
         case t @ AppliedType(tpe, typeList) =>
            import dotty.tools.dotc.util.Chars._
            if !t.typeSymbol.name.forall(
              isIdentifierPart
            ) && typeList.size == 2 then
               inner(typeList.head)
                  ++ texts(" ")
                  ++ inner(tpe)
                  ++ texts(" ")
                  ...
                  case Seq(arg, rtpe) =>
                     val partOfSignature = arg match
                        case byName: ByNameType =>
                           texts("(") ++ inner(byName) ++ texts(")")
                        case _ => inner(arg)
                     partOfSignature ++ texts(" => ") ++ inner(rtpe)
                  case args =>
                     texts("(") ++ commas(args.init.map(inner)) ++ texts(
                       ") => "
                     ) ++ inner(args.last)
               ...
               inner(tpe) ++ texts("[") ++ commas(typeList.map { t =>
                  t match
         ...
         case MatchType(bond, sc, cases) =>
            val casesTexts = cases.flatMap {
               case MatchCase(from, to) =>
                  texts("  case ") ++ inner(from) ++ texts(" => ") ++ inner(
                    to
                  ) ++ texts("\n")
   ...
   private def typeBound(t: TypeRepr, low: Boolean) =
      val ignore =
         if (low) t.typeSymbol == defn.NothingClass
         else t.typeSymbol == defn.AnyClass
   ...
   private def typeBoundsTreeOfHigherKindedType(low: TypeRepr, high: TypeRepr) =
      ...
      high.match
         case TypeLambda(params, paramBounds, resType) =>
            ...
            else regularTypeBounds(low, high)
