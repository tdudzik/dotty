      ...
      case r: Refinement => { //(parent, name, info)
        ...
        def getParamList(m: MethodType): DocSignature =
          texts("(")
            ++ m.paramNames
              ...
              .reduceLeftOption((acc: DocSignature, elem: DocSignature) =>
                acc ++ texts(", ") ++ elem
              )
              .getOrElse(List())
            ++ texts(")")
        ...
        def parsePolyFunction(info: TypeRepr): DocSignature = info match {
          case t: PolyType =>
            ...
            texts("[") ++ paramBounds ++ texts("] => ") ++ paramList ++ texts(
              " => "
            ) ++ resType
          case other => noSupported(s"Not supported type in refinement $info")
        }
        ...
        val refinedElems = refinementInfo.tail.collect { case r: Refinement =>
          r
        }.toList
        val prefix =
