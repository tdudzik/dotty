...
object ScalaSignatureProvider:
   ...
   private def enumPropertySignature(
       entry: Member,
       builder: SignatureBuilder
   ): SignatureBuilder =
   ...
   private def parentsSignature(
      ...
      member.directParents match
         ...
         case extendType :: withTypes =>
            val extendPart =
               builder.text(" extends ").signature(extendType.signature)
            withTypes.foldLeft(extendPart)((bdr, tpe) =>
               bdr.text(" with ").signature(tpe.signature)
            )
   ...
   private def givenMethodSignature(
       ...
       body: Kind.Def,
       builder: SignatureBuilder
   ): SignatureBuilder = method.kind match
      case Kind.Given(_, Some(instance), _) =>
         builder
            .text("given ")
   ...
   private def typeSignature(
      ...
      val bdr = builder
         .modifiersAndVisibility(
           typeDef,
           if tpe.opaque then "opaque type" else "type"
         )
   ...
   private def givenPropertySignature(
       property: Member,
       builder: SignatureBuilder
   ): SignatureBuilder =
      val bdr = builder
