...
case class InlineSignatureBuilder(
    names: Signature = Nil,
    preName: Signature = Nil
) extends SignatureBuilder:
   override def text(str: String): SignatureBuilder = copy(names = str +: names)
   override def name(str: String, dri: DRI): SignatureBuilder =
      copy(names = Nil, preName = names)
   override def driLink(text: String, dri: DRI): SignatureBuilder =
      copy(names = Link(text, dri) +: names)
   override def signature(s: Signature): SignatureBuilder =
      copy(names = s.reverse ++ names)

object InlineSignatureBuilder:
   def typeSignatureFor(d: Member): Signature =
      ScalaSignatureProvider
         .rawSignature(d, InlineSignatureBuilder())
...
trait SignatureBuilder extends ScalaSignatureUtils {
   ...
   def list[E](
      ...
      case head :: tail =>
         tail
            .foldLeft(elemOp(text(prefix), head))((b, e) =>
               elemOp(b.text(separator), e)
            )
            .text(suffix)
   }
   ...
   private def buildAnnotation(a: Annotation): SignatureBuilder =
      text("@")
         .driLink(a.dri.location.split('.').last, a.dri)
   ...
   private def buildAnnotationParams(a: Annotation): SignatureBuilder =
      if !a.params.isEmpty then
         ...
         list(params, "(", ")", ", ") { (bdr, param) =>
            bdr.buildAnnotationParameter(param)
         }
      else this
   ...
   def generics(on: Seq[TypeParameter]) = list(on.toList, "[", "]") {
      (bdr, e) =>
         bdr
            .annotationsInline(e)
            .text(e.variance)
            .memberName(e.name, e.dri)
            .signature(e.signature)
   }

   def functionParameters(params: Seq[ParametersList]) =
      if params.isEmpty then this.text("")
      else if params.size == 1 && params(0).parameters == Nil then
         this.text("()")
      else
         this.list(params, separator = "") { (bld, pList) =>
            bld.list(pList.parameters, s"(${pList.modifiers}", ")") {
               (bld, p) =>
                  val annotationsAndModifiers = bld
                     .annotationsInline(p)
                     .text(p.modifiers)
                  val name = p.name.fold(annotationsAndModifiers)(
                    annotationsAndModifiers.memberName(_, p.dri).text(": ")
                  )
