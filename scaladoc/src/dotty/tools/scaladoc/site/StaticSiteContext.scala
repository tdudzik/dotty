...
class StaticSiteContext(
   ...
   def indexTemplate(): Seq[LoadedTemplate] =
      val files =
         List(new File(root, "index.html"), new File(root, "index.md")).filter {
            _.exists()
         }

      if files.size > 1 then
         val msg =
            s"ERROR: Multiple root index pages found: ${files.map(_.getAbsolutePath)}"
         report.error(msg)
   ...
   lazy val sideBarConfig =
      ...
      if (!Files.exists(sidebarFile)) None
         ...
         Some(
           Sidebar.load(Files.readAllLines(sidebarFile).asScala.mkString("\n"))
         )

   lazy val templates: Seq[LoadedTemplate] =
      sideBarConfig.fold(loadAllFiles().sortBy(_.templateFile.title))(
        _.map(loadSidebarContent)
      )
   ...
   lazy val orphanedTemplates: Seq[LoadedTemplate] = {
      ...
      val allPaths =
         if !Files.exists(docsPath) then Nil
            ...
            Files
               .walk(docsPath, FileVisitOption.FOLLOW_LINKS)
               .iterator()
               .asScala
               .toList

      val orphanedFiles = allPaths
         .filterNot { p =>
   ...
   private def loadTemplate(
      ...
      if (!isValidTemplate(from)) None
      else
         try
            val topLevelFiles =
               if isBlog then Seq(from, new File(from, "_posts")) else Seq(from)
            val allFiles =
               topLevelFiles.filter(_.isDirectory).flatMap(_.listFiles())
            val (indexes, children) = allFiles
               .flatMap(loadTemplate(_, isBlog))
               .partition(_.templateFile.isIndexPage())

            def loadIndexPage(): TemplateFile =
               val indexFiles = from.listFiles { file =>
                  file.getName == "index.md" || file.getName == "index.html"
               }
               indexes match
                  case Nil => emptyTemplate(from, from.getName)
                  case Seq(loadedTemplate) =>
                     loadedTemplate.templateFile.copy(file = from)
                  case _ =>
                     // TODO (https://github.com/lampepfl/scaladoc/issues/238): provide proper error handling
                     val msg =
                        s"ERROR: Multiple index pages for $from found in ${indexes
                           .map(_.file)}"
                     throw new java.lang.RuntimeException(msg)

            val templateFile =
               if (from.isDirectory) loadIndexPage() else loadTemplateFile(from)

            def dateFrom(
                p: LoadedTemplate,
                default: String = "1900-01-01"
            ): String =
               val pageSettings = p.templateFile.settings.get("page").collect {
            ...
            val processedChildren: Seq[LoadedTemplate] =
               if !isBlog then children.sortBy(_.templateFile.title)
               else children.sortBy(dateFrom(_)).reverse
            ...
            val processedTemplate = // Set provided name as arg in page for `docs`
               if from.getParentFile.toPath == docsPath && templateFile
                  .isIndexPage() then
                  if templateFile.title != "index" then
                     report.warn(
                       "Property `title` will be overriden by project name",
                       from
                     )
            ...
            Some(
              LoadedTemplate(processedTemplate, processedChildren.toList, from)
            )
         catch
   ...
   private def loadSidebarContent(entry: Sidebar): LoadedTemplate = entry match
      case Sidebar.Page(title, url) =>
         ...
         val path =
            if isBlog then "blog"
            else if Files.exists(root.toPath.resolve(url)) then url
            else url.stripSuffix(".html") + ".md"

         val file = root.toPath.resolve(path).toFile
         val LoadedTemplate(template, children, _) =
            loadTemplate(
              file,
              isBlog
            ).get // Add proper logging if file does not exisits
         LoadedTemplate(
           template
              .copy(
                settings = template.settings + ("title" -> title),
                file = file
              ),
           children,
           file
   ...
   private def loadAllFiles() =
      ...
      dir("docs")
         .flatMap(_.listFiles())
         .flatMap(loadTemplate(_, isBlog = false))
   ...
   def driForLink(template: TemplateFile, link: String): Seq[DRI] =
      val pathsDri: Option[Seq[DRI]] = Try {
         ...
         baseFile.getFileName.toString.split("\\.").headOption.toSeq.flatMap {
            baseFileName =>
               Seq(
         ...
         if rawFilePath.startsWith(Paths.get("blog", "_posts")) then
            ...
            pageName.toString match
               case regex(year, month, day, name) =>
                  rawFilePath.getParent.resolveSibling(
                    Paths.get(year, month, day, name)
                  )
               case _ =>
                  val msg =
                     s"Relative path for blog: $rawFilePath doesn't match `yyy-mm-dd-name.md` format."
                  report.warn(msg, dest.toFile)
                  rawFilePath.resolveSibling(pageName.substring(0, dotIndex))
         else if (dotIndex < 0) rawFilePath.resolve("index")
         else rawFilePath.resolveSibling(pageName.substring(0, dotIndex))

      DRI.forPath(relativePath)

   def relativePath(myTemplate: LoadedTemplate) =
