...
class StaticSiteContext(
  ...
  lazy val sideBarConfig =
    ...
    if (!Files.exists(sidebarFile)) None
      ...
      Some(Sidebar.load(Files.readAllLines(sidebarFile).asScala.mkString("\n")))
        ...
        def loadIndexPage(): TemplateFile =
          ...
          indexes match
            ...
            case _ =>
              ...
              val msg =
                s"ERROR: Multiple index pages for $from found in ${indexes.map(_.file)}"
  ...
  private def loadSidebarContent(entry: Sidebar): LoadedTemplate = entry match
    case Sidebar.Page(title, url) =>
      ...
      LoadedTemplate(
        template
          .copy(settings = template.settings + ("title" -> title), file = file),
        children,
        file
      )
