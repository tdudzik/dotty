...
class SealedMarksGraphTransformer(using context: DocContext)
   ...
   override def apply(original: Module): Module =
      ...
      original.updateMembers { m =>
         m.copy(graph = m.graph.copy(sealedNodes = m.graph.vertecies.filter {
            v =>
   ...
   private def getSealedRelations(c: Member): Set[DRI] =
      val selfMapping =
         if c.modifiers.contains(Modifier.Sealed) then Set(c.dri) else Set.empty
