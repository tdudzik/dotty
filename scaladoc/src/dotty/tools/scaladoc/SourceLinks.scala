...
case class WebBasedSourceLink(prefix: String, revision: String, subPath: String)
    extends SourceLink:
   override val path: Option[Path] = None
   override def render(
       memberName: String,
...
class SourceLinkParser(revision: Option[String]) extends ArgParser[SourceLink]:
   ...
   def githubPrefix(org: String, repo: String) =
      ...
      val res = string match
         case KnownProvider(
            ...
            def withRevision(template: String => SourceLink) =
               pathRev.fold(Left(s"No revision provided"))(r =>
                  Right(template(r))
               )
...
class SourceLinks(val sourceLinks: PathBased[SourceLink]):
   def pathTo(
      ...
      sourceLinks
         .get(rawPath)
         .map(res => res.elem.render(memberName, res.path, operation, line))
