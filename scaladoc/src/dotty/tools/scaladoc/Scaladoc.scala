...
object Scaladoc:
   ...
   def run(args: Array[String], rootContext: CompilerContext): Reporter =
      ...
      parsedArgsOrNone.map { parsedArgs =>
         ...
         val tastyFiles =
            parsedArgs.tastyFiles ++ parsedArgs.tastyDirs.flatMap(
              listTastyFiles
            )
   ...
   def extract(
      ...
      val argumentFilesOrNone =
         ScaladocCommand.checkUsage(summary, true)(using ss)(using
           summary.sstate
         )
            ...
            newContext.settings.allSettings
               ...
               .fold(
                 report.warning(s"Unable to set ${s.name} in global context")
                  ...
                  newContext.setSetting(s.asInstanceOf[Setting[T]], newValue)
               )
         }
      ...
      argumentFilesOrNone.fold((None, newContext)) { argumentFiles =>
         ...
         if other.nonEmpty then
            report.warning(
              s"scaladoc suports only .tasty and .jar files, following files will be ignored: ${other
                 .mkString(", ")}"
            )
