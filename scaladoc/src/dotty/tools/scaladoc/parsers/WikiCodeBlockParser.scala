      ...
      def tryStart(
         ...
         if state.getIndent < 4 then {
            ...
            if matcher.find then {
               ...
               return BlockStart
                  .of(blockParser)
                  .atIndex(nextNonSpace + fenceLength)
            }
         }
         BlockStart.none
      }
...
class WikiCodeBlockParser(
      ...
      if matches then {
         ...
         if matcher.find then {
            ...
            if (foundFenceLength >= fenceLength) { // closing fence - we're at end of line, so we can finalize now
               block.setClosingMarker(
                 trySequence.subSequence(0, foundFenceLength)
               )
   ...
   override def isPropagatingLastBlankLine(
       lastMatchedBlockParser: BlockParser
   ) =
      false
   ...
   override def closeBlock(state: ParserState): Unit = { // first line, if not blank, has the info string
      ...
      if (lines.size > 0) {
         ...
         val spanningChars =
            chars.baseSubSequence(
              chars.getStartOffset,
              lines.get(0).getEndOffset
            )
