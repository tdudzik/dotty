...
trait SiteRenderer(using DocContext) extends Locations:
   ...
   def siteContent(pageDri: DRI, content: ResolvedTemplate): AppliedTag =
      ...
      def tryAsDri(str: String): Option[String] =
         ...
         val res =
            ctx.driForLink(content.template.templateFile, path)
               .filter(driExists)
         ...
         Try(URL(str))
            ...
            .orElse {
               Option.when(
                 Files.exists(
                   Paths
                      .get(content.ctx.root.toPath.toAbsolutePath.toString, str)
                 )
               )(
      ...
      document
         ...
         .forEach(element =>
            element
               .attr("href", processLocalLinkWithGuard(element.attr("href")))
         )
