...
class HtmlRenderer(rootPackage: Member, val members: Map[DRI, Member])(using
    ctx: DocContext
      ...
      staticSite match
         ...
         case Some(siteContext) =>
            val (indexes, templates) = siteContext.templates.partition(f =>
               f.templateFile
                  .isIndexPage() && f.file.toPath
                  .getParent() == siteContext.docsPath
               )
            if (indexes.size > 1)
               val msg =
            ...
            val templatePages =
               (templates ++ siteContext.indexTemplate())
            ...
            indexes.headOption match
               ...
               case None =>
                  Page(
                    Link(args.name, docsRootDRI),
                    "",
                    templatePages :+ rootPckPage.withTitle("API")
                  )
               case Some(indexPage) =>
                  val newChildren =
                     templatePages :+ rootPckPage.withTitle("API")
                  templateToPage(indexPage, siteContext).withNewChildren(
                    newChildren
                  )
   ...
   val hiddenPages: Seq[Page] =
      staticSite match
         case None =>
            Seq(
              navigablePage
                 .copy( // Add index page that is a copy of api/index.html
   ...
   def renderPage(page: Page, parents: Vector[Link]): Seq[String] =
      ...
      write(page.link.dri, content) +: page.children.flatMap(
   ...
   private def renderResources(): Seq[String] =
      ...
      def harvestResources(path: String) =
         ...
         if !Files.exists(siteImgPath) then Nil
            ...
            val files =
               allPaths.filter(Files.isRegularFile(_)).iterator().asScala
      ...
      val siteResourcesPaths =
         allPages.toSet.flatMap(specificResources) ++ staticResources

      val resources =
         siteResourcesPaths.toSeq.map(pathToResource) ++ allResources(allPages)
   ...
   def mkHead(page: Page): AppliedTag =
      val resources = page.content match
         case t: ResolvedTemplate =>
            t.resolved.resources ++ (if t.hasFrame then memberResourcesPaths
                                     else Nil)
         case _ =>
   ...
   private def buildNavigation(pageLink: Link): AppliedTag =
      def navigationIcon(member: Member) = member match {
         case m if m.needsOwnPage =>
            Seq(span(cls := s"micon ${member.kind.name.head}"))
         case _ => Nil
      }
      ...
      def renderNested(
         ...
         def linkHtml(exapnded: Boolean = false) =
            val attrs =
               if (isSelected || toplevel) Seq(cls := "selected expanded")
               else Nil
            ...
            Seq(
              a(href := pathToPage(pageLink.dri, nav.link.dri), attrs)(
                icon,
                nav.link.name
              )
            )
         ...
         nav.children match
            ...
            case children =>
               ...
               val attr =
                  if expanded || isSelected || toplevel then
                     Seq(cls := "expanded")
                  else Nil
   ...
   private def socialLinks(whiteIcon: Boolean = true) =
      val icon = (link: SocialLinks) =>
         if whiteIcon then link.whiteIconName else link.blackIconName
        ...
        div(id := "main")(
          ...
          footer(
            ...
            div(id := "generated-by")(
              raw("Generated by&nbsp;"),
              a(
                href := "https://github.com/lampepfl/dotty/tree/master/scaladoc"
              )(
                img(
