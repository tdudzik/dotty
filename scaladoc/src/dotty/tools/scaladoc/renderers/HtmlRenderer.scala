...
class HtmlRenderer(rootPackage: Member, val members: Map[DRI, Member])(using
  ...
  val hiddenPages: Seq[Page] =
    staticSite match
      case None =>
        Seq(
          navigablePage.copy( // Add index page that is a copy of api/index.html
  ...
  private def buildNavigation(pageLink: Link): AppliedTag =
    ...
    def renderNested(
      ...
      def linkHtml(exapnded: Boolean = false) =
        val attrs =
          if (isSelected || toplevel) Seq(cls := "selected expanded") else Nil
      ...
      nav.children match
        ...
        case children =>
          ...
          val attr =
            if expanded || isSelected || toplevel then Seq(cls := "expanded")
            else Nil
