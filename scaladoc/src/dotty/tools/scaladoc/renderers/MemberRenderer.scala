...
class MemberRenderer(signatureRenderer: SignatureRenderer)(using DocContext)
   ...
   def inheritedFrom(m: Member) = m.inheritedFrom match
      case Some(InheritedFrom(name, dri)) =>
         tableRow("Inherited from", signatureRenderer.renderLink(name, dri))
      case _ => Nil
   ...
   def memberSignature(member: Member) =
      ...
      val rawBuilder =
         ScalaSignatureProvider.rawSignature(member, InlineSignatureBuilder())
      ...
      Seq(
        span(cls := "modifiers")(
          span(cls := "other-modifiers")(
            modifiersRevered.reverse.map(renderElement)
          ),
   ...
   def memberIcon(member: Member) = member.kind match {
      ...
      case _ =>
         ...
         val iconSpan =
            span(cls := s"micon ${member.kind.name.head}$withCompanion")()
         Seq(
           member.companion
              .flatMap(link(_))
   ...
   def annotations(member: Member) =
      ...
      val signatures =
         rawBuilder.asInstanceOf[InlineSignatureBuilder].names.reverse
   ...
   def member(member: Member) =
      ...
      div(topLevelAttr: _*)(
        a(
          href := (if member.needsOwnPage then link(member.dri).getOrElse("#")
                   else s"#${member.dri.anchor}"),
          cls := "documentableAnchor"
        ),
        ...
        div(cls := "docs")(
          ...
          div(
            ...
            div(cls := "memberDocumentation")(
              memberInfo(member, withBrief = true)
            )
          )
        )
      )
   ...
   def grouppedMember(m: Member, membersInGroups: Seq[Member]): Tab =
      if membersInGroups.isEmpty then emptyTab
         ...
         val rawGroups =
            membersInGroups.groupBy(_.docs.flatMap(_.group)).collect {
        ...
        renderTabs(
          ...
          buildGroup(
            ...
            Seq(
              (
                "Defined exports",
                rest.filter(_.kind.isInstanceOf[Kind.Exported])
              )
            )
          )
        )
      ...
      if !m.kind.isInstanceOf[Classlike] then Nil
         ...
         def signatureList(list: Seq[LinkToType]): Seq[AppliedTag] =
            if list.isEmpty then Nil
               ...
               Seq(
                 div(cls := "symbol monospace")(
                   list.map(link =>
                      div(
                        link.kind.name,
                        " ",
                        link.signature.map(renderElement)
                      )
                   )
                 )
               )
