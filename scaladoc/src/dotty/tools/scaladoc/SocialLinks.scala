...
enum SocialLinks(
    val url: String,
    val whiteIconName: String,
    val blackIconName: String
):
   case Github(ghUrl: String)
       extends SocialLinks(
         ghUrl,
         "github-icon-white.png",
         "github-icon-black.png"
       )
   case Twitter(tUrl: String)
       extends SocialLinks(
         tUrl,
         "twitter-icon-white.png",
         "twitter-icon-black.png"
       )
   case Gitter(gUrl: String)
       extends SocialLinks(
         gUrl,
         "gitter-icon-white.png",
         "gitter-icon-black.png"
       )
   case Discord(dUrl: String)
       extends SocialLinks(
         dUrl,
         "discord-icon-white.png",
         "discord-icon-black.png"
       )
   case Custom(cUrl: String, cWhiteIconName: String, cBlackIconName: String)
       extends SocialLinks(cUrl, cWhiteIconName, cBlackIconName)
...
object SocialLinks:
   def parse(s: String): Either[String, SocialLinks] =
      ...
      splitted.head match {
         case "custom" if splitted.size == 4 =>
            Right(Custom(splitted(1), splitted(2), splitted(3)))
         case "custom" =>
            Left(
              errorPrefix + "For 'custom' arg expected three arguments: url, white icon name and black icon name"
            )
