...
class BaseHtmlTest:
  val unresolvedLinkSelector =
    ".documentableBrief span[data-unresolved-link], .cover span[data-unresolved-link]"
  ...
  def withGeneratedDoc(pcks: Seq[String], docsRoot: Option[String] = None)(
  ...
  class DocumentContext(d: Document, path: Path):
    ...
    def assertTextsIn(selector: String, expected: String*) =
      ...
      assertEquals(
        niceMsg(s"Context does not match for '$selector'"),
        expected.toList,
        found.toList
      )
