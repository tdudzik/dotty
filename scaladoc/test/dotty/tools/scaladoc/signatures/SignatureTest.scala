...
abstract class SignatureTest(
   ...
   def runTest = {
      ...
      afterRendering {
         ...
         val unexpectedFromSources: Set[String] =
            allSignaturesFromSources.collect { case Unexpected(name) =>
               name
            }.toSet
         ...
         val expectedButNotFound = expectedFromSources.flatMap { case (k, v) =>
            findMissingSingatures(v, actualSignatures.getOrElse(k, Nil))
         }
   ...
   private def findMissingSingatures(
       expected: Seq[String],
       actual: Seq[String]
   ): Set[String] =
         ...
         val content = document.select(".documentableElement").forEach { elem =>
            ...
            val other =
               elem
                  ...
                  .mkString("")
            val kind =
               elem.select(".header .kind").eachText.asScala.mkString("")
            val name =
               elem
            ...
            val sigPrefix = elem.select(".header .signature").textNodes match
               ...
               case _ => ""
            val all =
               s"$annotations$other $kind $name$sigPrefix$signature".trim()
            signatures += all
         }
