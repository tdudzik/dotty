...
class HtmlTagsTest {
   ...
   def divWithTextInside = {
      val actual =
         div(h1(), span("Some text"), "Some more of the text").toString
      val expect =
   ...
   def appliedAttrAndSeqAppliedAttr = {
      val actual =
         div(cls := "someClass", Seq(style := "some: style;")).toString
   ...
   def seqAppliedAttrAndAppliedAttr = {
      val actual =
         div(Seq(cls := "someClass"), style := "some: style;").toString
      val expect = """<div class="someClass" style="some: style;"></div>"""
      assertEquals(expect, actual)
   }
   ...
   def nilAmongArgs = {
      val actual =
         div(cls := "someClass", Nil, style := "some: style;").toString
      val expect = """<div class="someClass" style="some: style;"></div>"""
      assertEquals(expect, actual)
   }
